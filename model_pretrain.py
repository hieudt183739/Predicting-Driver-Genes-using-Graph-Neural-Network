...
# tinh ma tran va ma tran transpose: Ppr va Prp
...
class graph_inception(torch.nn.Module):
    ...
    def forward(self, l_feat, r_feat, network):
        Korder_list = [[l_feat],[r_feat]]    # thuoc tinh node ban dau: [Xp, Xr]
        convresults = [[] for _ in range(2)] # thuoc tinh node sau khi pretrain
        l_index = 0
        r_index = 1
        network_l, network_r = sparse_to_matrix(network)   # Ppr, Prp
        for i in range(self.hop):
            temp_Korders = [[] for _ in range(2)]

            # l_node
            x0 = F.dropout(Korder_list[r_index][i], training=self.training)
            l_reFeat, l_Korder = graph_inception_unit(network_l, x0, self.lin1)    #  Neighourhood Aggregator Feature cho gene node
            ...
            r_reFeat, r_Korder = graph_inception_unit(network_r, x1, self.lin1)    #  Neighourhood Aggregator Feature cho miRNA node
            convresults[r_index].append(r_reFeat)
            temp_Korders[r_index].append(r_Korder)

            # Dot product feature with neighbors
            l_mul, l_Korder_mul = Multiply(network_l, x0, x1, self.lin2)            # Neighbourhood Aggregator Interaction cho gene node
            r_mul, r_Korder_mul = Multiply(network_r, x1, x0, self.lin2)            # Neighbourhood Aggregator Interaction cho miRNA node
...
class pretrain(torch.nn.Module):
    ...
    def forward(self):
        ...
        # gene-miRNA graph
        r_feature = self.lin2(self.r_feat)      
